...
def draw_boxes_fade(
    ...
    # fillPoly wants polys in a sequence with points inside as (x,y)
    box_raster_coords = cv2_subpixel(box_raster_coords.reshape((-1, 4, 2)))
    cv2.fillPoly(base_image, box_raster_coords, color=color, **CV2_SUB_VALUES)
    return base_image
...
class BoxRasterizer(Rasterizer):
    ...
    def rasterize(
        ...
        fade_factor = 5
        agents_image = np.zeros((self.raster_size[1], self.raster_size[0]), dtype=np.uint8)
        ego_image = np.zeros((self.raster_size[1], self.raster_size[0]), dtype=np.uint8)
        for i, (frame, agents) in enumerate(zip(reversed(history_frames), reversed(history_agents))):
            ...
            agents_image = draw_boxes_fade(self.raster_size, raster_from_world, agents, 255-(len(history_frames)-i)*fade_factor, agents_image)
            if len(ego_agent) > 0 and (self.render_ego_history or i == 0):
                ego_image = draw_boxes_fade(self.raster_size, raster_from_world, ego_agent, 255-(len(history_frames)-i)*fade_factor, agents_image)

            # agents_images.append(draw_boxes(self.raster_size, raster_from_world, agents, 255))
            # if len(ego_agent) > 0 and (self.render_ego_history or i == 0):
            #     ego_images.append(draw_boxes(self.raster_size, raster_from_world, ego_agent, 255))

        # if len(agents_images) != (self.history_num_frames + 1):
        #     missing_frame = self.history_num_frames + 1 - len(agents_images)
        #     for i in range(missing_frame):
        #         agents_images.append(np.zeros((self.raster_size[1], self.raster_size[0]), dtype=np.uint8))


        # if len(ego_images) != (self.history_num_frames + 1):
        #     missing_frame = self.history_num_frames + 1 - len(ego_images)
        #     for i in range(missing_frame):
        #         ego_images.append(np.zeros((self.raster_size[1], self.raster_size[0]), dtype=np.uint8))

        # combine such that the image consists of [agent_t, agent_t-1, agent_t-2, ego_t, ego_t-1, ego_t-2]

        

        agents_images.append(agents_image)
        ego_images.append(ego_image)

        out_im = np.asarray(agents_images + ego_images, dtype=np.uint8)
