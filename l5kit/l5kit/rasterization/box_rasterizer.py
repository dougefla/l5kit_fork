...
class BoxRasterizer(Rasterizer):
    ...
    def to_rgb(self, in_im: np.ndarray, **kwargs: dict) -> np.ndarray:
        ...
        """
        # in_im = np.transpose(in_im, (2, 1, 0))
        hist_frames = in_im.shape[0] // 2
        ...
        for i in range(self.raster_size[1]):
            for j in range(self.raster_size[0]):
                if out_im_ego[i,j][1]:
                    out_im[i,j] = out_im_ego[i,j]
        return (np.clip(out_im, 0, 1) * 255).astype(np.uint8)
